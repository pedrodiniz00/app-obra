...
# 6. ABA PAGAMENTOS (ATUALIZADA COM CAMPOS DE ORÇAMENTO)
with tabs[5]:
    st.subheader("💰 Gestão Financeira")
    
    # NOVOS CAMPOS: Definir Orçamentos
    with st.expander("⚙️ Definir Orçamentos da Obra", expanded=False):
        ...
        nv_orc_p = c_orc2.number_input("Orçamento Pedreiro (Mão de Obra) (R$)", value=orc_p_db, step=100.0, format="%.2f")
    ...
    res1.metric("Recebido do Cliente", formatar_moeda(r_cl['total'].sum()))
    res2.metric("Pago ao Pedreiro", formatar_moeda(p_m['total'].sum()))
    res3.metric("A Pagar (Pedreiro)", formatar_moeda(orc_p_db - p_m['total'].sum()))
    res4.metric("Saldo com Cliente", formatar_moeda(orc_c_db - r_cl['total'].sum()))
    ...
    with st.form("f_p", clear_on_submit=True):
        ...
        v_l = cp3.number_input("Valor", 0.0, format="%.2f")
    ...
    with c_saida: 
        st.error("🔴 Detalhe Saídas")
        p_m_edit = st.data_editor(p_m[['id', 'data', 'descricao', 'total']], key="eds", hide_index=True, num_rows="dynamic", use_container_width=True, column_config={"total": st.column_config.NumberColumn(format="R$ %.2f")})
    with c_entrada:
        st.success("🟢 Detalhe Entradas")
        r_cl_edit = st.data_editor(r_cl[['id', 'data', 'descricao', 'total']], key="ede", hide_index=True, num_rows="dynamic", use_container_width=True, column_config={"total": st.column_config.NumberColumn(format="R$ %.2f")})
    
    if st.button("💾 Sincronizar Listas"):
