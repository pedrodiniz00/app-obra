...
with st.sidebar:
    ...
    if not DB['obras'].empty:
        # Garantir que a coluna arquivada existe e é booleana
        ...
        for i, pai in enumerate(sorted(crono_f['pai'].unique()), 1):
            ...
            with exp:
                with st.popover("➕ Add"):
                    ...
                    if st.button("Salvar", key=f"bas_{i}"):
...
with tabs[5]:
    ...
    with st.expander("⚙️ Definir Orçamentos", expanded=False):
        ...
        nv_c = c_orc1.number_input("Cliente R$", value=orc_c_db, format="%.2f")
        nv_p = c_orc2.number_input("Pedreiro R$", value=orc_p_db, format="%.2f")
