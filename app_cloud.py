...
# --- 1. CONEXÃO COM CACHE (Só conecta 1 vez) ---
@st.cache_resource
...
# --- 2. LEITURA COM CACHE (Guarda dados por 60 segs) ---
...
def carregar_dados_gerais():
    ...
    try:
        ...
        # Se der erro (aba não existe), retorna vazio
        df_crono = pd.DataFrame()
        
    return df_custos, df_crono

# Função auxiliar para pegar as abas "vivas" para escrita (sem cache)
...
# Carrega os dados da memória (rápido e não gasta cota)
df_custos, df_cronograma = carregar_dados_gerais()

# Se o cronograma estiver vazio no cache, força uma recarga inicial
if df_cronograma.empty:
    st.info("Carregando estrutura inicial...")
    # Lógica simples de fallback se for a primeira vez
    ...
    if len(ws_cr.get_all_values()) < 2:
        ...
        st.cache_data.clear() # Limpa cache para ler certo na proxima
        st.rerun()
...
if not df_cronograma.empty:
    ...
    for i, item in enumerate(lista_etapas):
        ...
        # Se mudou, aí sim conectamos para salvar
        if novo_check != status_bool:
            ...
            st.cache_data.clear() # Limpa a memória para ver a mudança
            st.rerun()
...
with st.expander("💰 Novo Gasto", expanded=True):
    ...
    # Tenta pegar lista de etapas, se vazio usa padrão
    ...
    if st.button("Salvar Gasto"):
        ...
        st.success("Salvo!")
        st.cache_data.clear() # Limpa a memória para ver o novo gasto
        st.rerun()
